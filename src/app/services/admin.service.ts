...
export class AdminService {
  ...
  actualizarMicrociclo(microciclo: Microciclo, clienteId: string) {
    let microSave: Microciclo = {
      id: microciclo.id,
      nro: microciclo.nro,
      fecha_inicio: microciclo.fecha_inicio,
      fecha_fin: microciclo.fecha_fin,
      plan_id: microciclo.plan_id,
      tipo: microciclo.tipo
    }
    return new Promise(resolve => {
      this.http.put(`${URL}/clientes/${clienteId}/planes/${microciclo.plan_id}/microciclos/${microciclo.id}`, microSave).subscribe(data => {
        console.log(data)
        if (data['ok']) {
          resolve(true);
        }
  ...
  getPlanAnterior(clienteId: string, tipo: string, numero: number){
    return this.http.get(`${URL}/clientes/${clienteId}/plan/${tipo}/${numero}?tipoPlan=${tipo}`)
  }
